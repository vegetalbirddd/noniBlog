...
JS阻塞DOM解析、CSS阻塞页面渲染、大图片阻塞首屏、长任务阻塞交互、网络弱网下全链路耗时翻倍


### 全链路系统化优化思路

网络层：DNS预解析、预连接、HTTP2/QUIC、CDN加速

缓存层：合理分级强缓存+协商缓存、内存缓存优先级利用

资源层：资源压缩、拆分JS、懒加载、图片格式优化

渲染层：减少强制同步布局、批量修改DOM、利用GPU合成、避免高频回流



### 线上故障定位思路

页面白屏：排查HTML加载失败、JS报错阻塞渲染、样式缺失

页面卡顿：排查主线程长任务、高频回流、无效重绘

加载缓慢：逐段拆解DNS/建连/请求/解析/渲染各阶段耗时定位瓶颈


### 工程化落地

基于web-vitals采集线上真实用户链路指标，接入监控平台，做分层性能告警
