import{_ as n,o as e,c as s,b as a}from"./app-c67da3eb.js";const i={},t=a(`<h2 id="对平台进行i18n国际化语言配置-解决国际化词条动态拼接、增量i18n提效等痛点" tabindex="-1"><a class="header-anchor" href="#对平台进行i18n国际化语言配置-解决国际化词条动态拼接、增量i18n提效等痛点" aria-hidden="true">#</a> 对平台进行i18n国际化语言配置，解决国际化词条动态拼接、增量i18n提效等痛点</h2><p>基于<strong>i18next</strong>体系，接入了腾讯<strong>ti18n</strong>平台，ti18n在i18next的基础上封装了CDN加载和CLI提取能力，实现翻译资源CDN化管理（翻译发布和代码解耦，翻译更新不用走CI/CD）。</p><p>工程化：ti18n CLI扫描源码，自动生成待翻译列表，防止遗漏</p><p>翻译管理：平台化写作，翻译人员无需接触代码仓库</p><p>降级策略：key未找到的时候显示原文</p><p>一个json语言包100多k</p><h3 id="单复数" tabindex="-1"><a class="header-anchor" href="#单复数" aria-hidden="true">#</a> 单复数</h3><p>i18next提供的plural</p><p>i18next根据传入count参数自动选择带_plural后缀的key</p><h3 id="不同国家时间展示" tabindex="-1"><a class="header-anchor" href="#不同国家时间展示" aria-hidden="true">#</a> 不同国家时间展示</h3><p>Intl.DateTimeFormat</p><h3 id="默认语言" tabindex="-1"><a class="header-anchor" href="#默认语言" aria-hidden="true">#</a> 默认语言</h3><p>先用navigator，存到localStorage</p><h3 id="不同语序高亮" tabindex="-1"><a class="header-anchor" href="#不同语序高亮" aria-hidden="true">#</a> 不同语序高亮</h3><div class="language-jsx line-numbers-mode" data-ext="jsx"><pre class="language-jsx"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span><span class="token class-name">Trans</span></span>
  <span class="token attr-name">i18nKey</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&quot;</span>welcome<span class="token punctuation">&quot;</span></span>
  <span class="token attr-name">values</span><span class="token script language-javascript"><span class="token script-punctuation punctuation">=</span><span class="token punctuation">{</span><span class="token punctuation">{</span>
  ...
  <span class="token attr-name">components</span><span class="token script language-javascript"><span class="token script-punctuation punctuation">=</span><span class="token punctuation">{</span><span class="token punctuation">{</span>
    <span class="token literal-property property">highlight</span><span class="token operator">:</span> <span class="token punctuation">(</span>
      <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>span</span> <span class="token attr-name">className</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&quot;</span>highlight<span class="token punctuation">&quot;</span></span> <span class="token punctuation">/&gt;</span></span>
    <span class="token punctuation">)</span>
  <span class="token punctuation">}</span><span class="token punctuation">}</span></span>
<span class="token punctuation">/&gt;</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="样式处理" tabindex="-1"><a class="header-anchor" href="#样式处理" aria-hidden="true">#</a> 样式处理</h3><div class="language-css line-numbers-mode" data-ext="css"><pre class="language-css"><code><span class="token selector">.input</span> <span class="token punctuation">{</span>
  <span class="token property">width</span><span class="token punctuation">:</span> <span class="token string">&quot;220px&quot;</span><span class="token punctuation">;</span>
  <span class="token property">work-break</span><span class="token punctuation">:</span> break-all<span class="token punctuation">;</span>
  <span class="token selector">// 加个作用域，限制生效范围，非 en 语言下就不生效。
  [lang=&quot;en&quot;] &amp;</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>💡 <strong>为什么能减少 60%+ 的重复色值代码？</strong></p><ol><li><p><strong>一处变更，全局生效：</strong> 以前每个组件的 hover、active、border 都要单独写死色值，现在全量归拢到 theme.css 的几十行代码中。</p></li><li><p><strong>消除样式分支：</strong> 以前做换肤普遍要在 React 里写 className={theme === &#39;dark&#39; ? &#39;btn-dark&#39; : &#39;btn-light&#39;}，导致 JS 和 CSS 里充斥着双份逻辑。现在<strong>组件不需要关心当前是什么主题</strong>，它只需要盲目信任 var(--color-primary) 即可，解耦极其彻底。</p></li></ol></blockquote><h2 id="useeffect打架导致render崩溃问题" tabindex="-1"><a class="header-anchor" href="#useeffect打架导致render崩溃问题" aria-hidden="true">#</a> useEffect打架导致render崩溃问题</h2><div class="language-jsx line-numbers-mode" data-ext="jsx"><pre class="language-jsx"><code><span class="token keyword">const</span> <span class="token punctuation">[</span>messages<span class="token punctuation">,</span> setMessages<span class="token punctuation">]</span> <span class="token operator">=</span> <span class="token function">useState</span><span class="token punctuation">(</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">)</span>
...
<span class="token function">useEffect</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=&gt;</span> <span class="token punctuation">{</span>
  <span class="token function">setScrollTop</span><span class="token punctuation">(</span>containerRef<span class="token punctuation">.</span>current<span class="token punctuation">.</span>scrollHeight<span class="token punctuation">)</span>
...
<span class="token function">useEffect</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=&gt;</span> <span class="token punctuation">{</span>
  <span class="token function">setMessages</span><span class="token punctuation">(</span><span class="token function">fixMessage</span><span class="token punctuation">(</span>messages<span class="token punctuation">)</span><span class="token punctuation">)</span>
<span class="token punctuation">}</span><span class="token punctuation">,</span> <span class="token punctuation">[</span>scrollTop<span class="token punctuation">]</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>本质上 useEffect 死循环通常是因为 Effect 监听某个状态，同时又在内部修改这个状态，或者依赖项引用不稳定（对象、数组、函数），导致每次 Render 都触发 Effect。解决思路一般是稳定依赖引用、拆分状态职责，或者避免 Effect 修改自身依赖的数据。</p>`,139),o=[t];function l(r,d){return e(),s("div",null,o)}const p=n(i,[["render",l],["__file","jianlixiangmujiedu.html.vue"]]);export{p as default};
